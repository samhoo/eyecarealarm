...
- **两阶段遮罩**（覆盖所有显示器）：
  ...
  - `5–6s` 第二阶段文字渐显（较原设计缩短 1 秒）
  - `8s` 起屏蔽全部输入并播放休息音效；正式阶段按控制面板设置的休息时长（5–60 秒）计时，结束前 2 秒渐隐；退出按钮在输入解锁后立即显示并可点击
  - `Esc` 或点击「退出」在正式阶段开始后立即提前结束（1 秒渐隐退出）
- **控制面板**：倒计时 + 今日放松次数、提醒间隔（1–120 分钟）、休息时长（5–60 秒）、遮罩透明度、音效选择与导入、音量、中/英双语、开机自启动
...
│   │   ├── overlay.rs     # 多屏遮罩编排：预创建窗口 + 设置时长状态机
